...
import { toggleCheckedTickerAC, updateIntervalAC } from '../../store/tickers/actionCreators'
            ...
            {tickers.map((item, index) => (
              ...
              >
                <StyledTableCell component='th' scope='row'>
                  <Switch onClick={() => dispatch(toggleCheckedTickerAC(item.ticker))}
                          checked={!item.disabled} size='small' />
                  <Box sx={{
                    fontSize: '1rem',
                    fontWeight: 'bold',
                    lineHeight: '1.5rem',
                ...
                <StyledTableCell align='center'>{item.disabled ? 'disabled' : ''}{item.exchange}</StyledTableCell>
                ...
                <StyledTableCell sx={{
                  ...
                  {item.isIncrease ? '🠕' : '🠗'}{item.change_percent}%</StyledTableCell>
                <StyledTableCell align='center'>{item.dividend}</StyledTableCell>
                <StyledTableCell align='center'>{item.yield}</StyledTableCell>
                <StyledTableCell align='center'>{dateGenerator(item.last_trade_time)}</StyledTableCell>
              </StyledTableRow>
            ))}
          </TableBody>
        </Table>
